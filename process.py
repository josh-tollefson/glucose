...
def read_csv(
    ...
    if skiprows < 10:
        raise ValueError("skiprows must be >=10 to not read in user, device, & alert info.")

    if len(cols) != len(col_names):
        raise ValueError("number of columns read must be equal to number of defined column names.")
...
def normalize(
    ...
    df[GLUCOSE_FIELD_ORIG] = (df[GLUCOSE_FIELD_ORIG] - min_glucose) / (max_glucose - min_glucose)
...
def get_processed_stats(
    ...
    if rows.shape[0] <= 1:
        ...
        diff_glucose = rows[GLUCOSE_FIELD_ORIG].iloc[-1] - rows[GLUCOSE_FIELD_ORIG].iloc[0]
...
def get_processed_df(
    ...
    rows = df.apply(lambda row: f(row[TIMESTAMP], row[LOW_FIELD], df, min_time=30, max_time=60, step=5, unit='m'), axis=1)
...
def main(
    ...
    processed = get_processed_df(df, method=method, savefile=outfile)
